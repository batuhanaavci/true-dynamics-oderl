...
class BaseEnv(gym.Env, metaclass=ABCMeta):
    ...
    def set_solver(self, method='rk4', rtol=1e-6, atol=1e-9, num_bins=None):
        if num_bins is None:
            if method=='euler':
                num_bins = 1000
            elif method=='rk4':
    ...
    def integrate_system(self, T, g, s0=None, N=1, return_states=False):
        ...
        st = odeint(odefnc, s0, ts, rtol=self.solver['rtol'], atol=self.solver['atol'], method=self.solver['method'])
