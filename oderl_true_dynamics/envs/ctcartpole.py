...
class CTCartpole(BaseEnv):
    ...
    def __init__(self, dt=0.1, device='cuda', swing_up=True, obs_trans=False, obs_noise=0.0, \
    ...
    def torch_transform_states(self,state):
        ...
        '''
        if self.obs_trans:
            print('Trigonometrically transformed states cannot be transformed!\n')
    ...
    #################### override ##################
    def reset(self):
        # after 9 times of sampling -0.05 to 0.05, sample 10th from -0.6 to 0.6 use self.counter
     
        rand_state  = np.random.uniform(low=-0.05, high=0.05, size=(4,))
    ...
    def torch_rhs(self, state, action):
        ...
        if fiveD:
            ...
            costheta = torch.cos(theta_safe)
            sintheta = torch.sin(theta_safe)
    ...
    def diff_obs_reward_(self,s,penalize_vel=False):
        if s.shape[-1]==4:
            ...
            theta_safe = theta.clone()
            cos_th, sin_th = torch.cos(theta_safe), torch.sin(theta_safe)
